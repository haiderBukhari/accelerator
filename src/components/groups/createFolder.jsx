...
export default function CreateFolderDialog({ open, setOpen, groupId, setFetchAgain }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_APP_BACKEND_URL}/groups-folders`, formData, {
                headers: {
                    ...
                    >
                        <div className="flex gap-2 text-lg font-bold leading-4 text-violet-800 whitespace-nowrap">
                            <img
                                ...
                                alt="Profile"
                            />
                            <div className="my-auto text-[17px] ml-0">Folder Picture</div>
                        </div>
                    </div>
                    <input
                        type="file"
                        ...
                        ref={fileInputRef}
                    />
                    <div className="w-full font-semibold mt-6">Folder Name</div>
                    <input onChange={(event) => { setName(event.target.value) }} value={name} style={{ border: "1px solid #AAAAAA" }} type="text" name="" id="" className="justify-center items-start px-5 py-2 mt-3 w-full text-base tracking-wider rounded-xl border border-solid outline-none text-black bg-zinc-300 border-stone-300" placeholder='i.e. Laptops' />
                </DialogContent>
                <DialogActions>
